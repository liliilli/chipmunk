...
impl Memory {
    pub fn new(valid_file_path: &str) -> Option<Memory> {
        ...
        let font_pack = 
        {
            let mut result = Vec::<u8>::with_capacity(16 * 5 * 2);
            result.extend_from_slice(&[0xF, 0x0, 0x9, 0x0, 0x9, 0x0, 0x9, 0x0, 0xF, 0x0]); // 0
            result.extend_from_slice(&[0x2, 0x0, 0x6, 0x0, 0x2, 0x0, 0x2, 0x0, 0x7, 0x0]); // 1
            result.extend_from_slice(&[0xF, 0x0, 0x1, 0x0, 0xF, 0x0, 0x8, 0x0, 0xF, 0x0]); // 2
            result.extend_from_slice(&[0xF, 0x0, 0x1, 0x0, 0xF, 0x0, 0x1, 0x0, 0xF, 0x0]); // 3
            result.extend_from_slice(&[0x9, 0x0, 0x9, 0x0, 0xF, 0x0, 0x1, 0x0, 0x1, 0x0]); // 4
            result.extend_from_slice(&[0xF, 0x0, 0x8, 0x0, 0xF, 0x0, 0x1, 0x0, 0xF, 0x0]); // 5
            result.extend_from_slice(&[0xF, 0x0, 0x8, 0x0, 0xF, 0x0, 0x9, 0x0, 0xF, 0x0]); // 6
            result.extend_from_slice(&[0xF, 0x0, 0x1, 0x0, 0x2, 0x0, 0x4, 0x0, 0x4, 0x0]); // 7
            result.extend_from_slice(&[0xF, 0x0, 0x9, 0x0, 0xF, 0x0 ,0x9, 0x0, 0xF, 0x0]); // 8
            result.extend_from_slice(&[0xF, 0x0, 0x9, 0x0, 0xF, 0x0, 0x1, 0x0, 0xF, 0x0]); // 9
            result.extend_from_slice(&[0xF, 0x0, 0x9, 0x0, 0xF, 0x0, 0x9, 0x0, 0x9, 0x0]); // A
            result.extend_from_slice(&[0xE, 0x0, 0x9, 0x0, 0xE, 0x0, 0x9, 0x0, 0xE, 0x0]); // B
            result.extend_from_slice(&[0xF, 0x0, 0x8, 0x0, 0x8, 0x0, 0x8, 0x0, 0xF, 0x0]); // C
            result.extend_from_slice(&[0xE, 0x0, 0x9, 0x0, 0x9, 0x0, 0x9, 0x0, 0xE, 0x0]); // D
            result.extend_from_slice(&[0xF, 0x0, 0x8, 0x0, 0xF, 0x0, 0x8, 0x0, 0xF, 0x0]); // E
            result.extend_from_slice(&[0xF, 0x0, 0x8, 0x0, 0xF, 0x0, 0x8, 0x0, 0x8, 0x0]); // F
            result
        };
